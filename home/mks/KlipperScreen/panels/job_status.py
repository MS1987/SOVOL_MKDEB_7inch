...
class JobStatusPanel(ScreenPanel):
    def __init__(self, screen, title):
        ...
        self._screen._ws.klippy.gcode_script("BED_MESH_PROFILE LOAD=default")
    ...
    def process_update(self, action, data):
        if action == "notify_gcode_response":
            ...
            print(f"action != notify_status_update: {action}")
            self.job_info_check()
            return
    ...
    def update_time_left(self, total_duration, print_duration, fila_used=0):
        ...
        with contextlib.suppress(ZeroDivisionError):
            file_time = (total_duration / self.progress if self.progress > 0 else 1)# + non_printing
