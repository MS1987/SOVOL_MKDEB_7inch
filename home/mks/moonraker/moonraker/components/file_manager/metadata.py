...
class BaseSlicer(object):
    ...
    def set_data(self,
                 ...
                 footer_data: str,
                 fsize: int) -> None:
        log_to_stderr("/************************BaseSlicer*********************/")
        log_to_stderr(f"self.header_data: {len(header_data)}")
        log_to_stderr(f"self.footer_data: {len(footer_data)}")
        log_to_stderr(f"self.size: {fsize}")
...
class Slic3rPE(PrusaSlicer):
    def check_identity(self, data: str) -> Optional[Dict[str, str]]:
        ...
        log_to_stderr(f"Slic3rPE match: {match}")
        if match:
            return {
                'slicer': "Slic3r PE",
                'slicer_version': match.group(1)
...
class Slic3r(Slic3rPE):
    def check_identity(self, data: str) -> Optional[Dict[str, str]]:
        ...
        log_to_stderr(f"Slic3r match: {match}")
        if match:
            return {
                'slicer': "Slic3r",
                'slicer_version': match.group(1)
...
class Cura(BaseSlicer):
    def check_identity(self, data: str) -> Optional[Dict[str, str]]:
        ...
        log_to_stderr(f"Cura match: {match}")
        if match:
            return {
                'slicer': "Cura",
                'slicer_version': match.group(1)
...
class Comgrow3DCura(BaseSlicer):
    def check_identity(self, data: str) -> Optional[Dict[str, str]]:
        ...
        log_to_stderr(f"header_data: {len(self.header_data)}")

        for name, expr in aliases.items():
            log_to_stderr(f"name: {name}, expr: {expr}")
            match = re.search(expr, data)
            log_to_stderr(f"match: {match}")
            if match:
                return {
                    'slicer': name,
                    'slicer_version': match.group(1)
                }
            log_to_stderr(f"match: {match} 'slicer': {name}, 'slicer_version': {match.group(1)}")
        return None

    def has_objects(self) -> bool:
        _has_object = self._check_has_objects(self.header_data, r"\n; printing object")
        log_to_stderr(f"_has_object: {_has_object}")
        return _has_object

    def parse_first_layer_height(self) -> Optional[float]:
        # Check percentage
        first_layer_height = float(self._regex_find_first(
            r";Layer height: \d+\.?\d+", self.header_data).replace(";Layer height: ", ""))
        log_to_stderr(f"3: {first_layer_height}")
        return first_layer_height

    def parse_layer_height(self) -> Optional[float]:
        self.layer_height = float(self._regex_find_first(
            r";Layer height: \d+\.?\d+", self.header_data).replace(";Layer height: ", ""))
        log_to_stderr(f"self.layer_height: {self.layer_height}\n")
        return self.layer_height

    def parse_object_height(self) -> Optional[float]:
        matches = re.findall(
            r";BEFORE_LAYER_CHANGE\n(?:.*\n)?;(\d+\.?\d*)", self.header_data)
        if matches:
            try:
                matches = [float(m) for m in matches]
            except Exception:
                ...
                log_to_stderr(f"max(matches): {max(matches)}")
                return max(matches)
        mac_float = self._regex_find_first(r";MAXZ:\d*\.?\d*", self.header_data)
        if mac_float:
            return float(mac_float.strip(";MAXZ:"))
        log_to_stderr(f"mac_float: {mac_float}\n")
        return mac_float

    def parse_filament_total(self) -> Optional[float]:
        filament_total = self._regex_find_first(r"Filament used: \d*\.?\d*m", self.header_data)
        if filament_total:
            return float(filament_total.strip("Filament used: ").strip("m"))
        log_to_stderr(f"filament_total: {filament_total}\n")
        return filament_total

    def parse_filament_weight_total(self) -> Optional[float]:
        filament_weight_total = self._regex_find_first(
            r"total\sfilament\sused\s\[g\]\s=\s(\d+\.\d*)", self.header_data)
        log_to_stderr(f"filament_weight_total: {filament_weight_total}\n")
        return filament_weight_total

    def parse_filament_type(self) -> Optional[str]:
        filament_type = _regex_find_string(
            r";\sfilament_type\s=\s(.*)", self.header_data)
        log_to_stderr(f"filament_type: {filament_type}\n")
        return filament_type

    def parse_filament_name(self) -> Optional[str]:
        filament_name = _regex_find_string(
            r";\sfilament_settings_id\s=\s(.*)", self.header_data)
        log_to_stderr(f"filament_name: {filament_name}\n")
        return filament_name

    def parse_estimated_time(self) -> Optional[float]:
        time_match = self._regex_find_first(r';TIME:\d+', self.header_data)
        log_to_stderr(f'time_match: {time_match}\n')
        if not time_match:
            log_to_stderr('None\n')
            return None
    ...
    def parse_first_layer_extr_temp(self) -> Optional[float]:
        first_layer_extr_temp = self._regex_find_first(r"M104 S\d*", self.header_data)
        if first_layer_extr_temp:
            return float(first_layer_extr_temp.replace("M104 S", ""))
        log_to_stderr(f"first_layer_extr_temp: {first_layer_extr_temp}\n")
        return first_layer_extr_temp

    def parse_first_layer_bed_temp(self) -> Optional[float]:
        bed_temp = self._regex_find_first(r"M140 S\d*", self.header_data)
        if bed_temp:
            return float(bed_temp.replace("M140 S", ""))
        log_to_stderr(f"first_layer_bed_temp:{bed_temp}\n")
        return bed_temp

    def parse_chamber_temp(self) -> Optional[float]:
        chamber_temp = self._regex_find_first(
            r"; chamber_temperature = (\d+\.?\d*)", self.header_data)
        log_to_stderr(f"chamber_temp: {chamber_temp}\n")
        return chamber_temp

    def parse_nozzle_diameter(self) -> Optional[float]:
        nozzle_diameter = self._regex_find_first(
            r";\snozzle_diameter\s=\s(\d+\.\d*)", self.header_data)
        log_to_stderr(f"nozzle_diameter: {nozzle_diameter}\n")
        return nozzle_diameter

    def parse_layer_count(self) -> Optional[int]:
        layer_count = _regex_find_int(
            r";LAYER_COUNT:(\d+)", self.header_data)
        log_to_stderr(f"layer_count: {layer_count}\n")
        return layer_count

    def _regex_find_first(self, pattern: str, data: str) -> Optional[str]:
        val = re.search(pattern, data)
        log_to_stderr(f"pattern: {pattern}, val: {val}")
        if not val:
            log_to_stderr('None\n')
            return None
        return val.group()
    ...
    def parse_thumbnails(self) -> Optional[List[Dict[str, Any]]]:
        ...
        if len(info) != 3:
            log_to_stderr("MetadataError: Error parsing thumbnail")

        if len(preview_data) != info[2]:
            log_to_stderr(
                f"MetadataError: Thumbnail Size Mismatch: "
        ...
        rel_thumb_path = os.path.join(".thumbs", thumb_name)
        with open(thumb_path, "wb") as f:
            f.write(base64.b64decode(preview_data.encode()))
        parsed_matches.append({
            'width': info[0], 'height': info[1],
            'size': os.path.getsize(thumb_path),
            'relative_path': rel_thumb_path})
        if info[0] == 160 and info[1] == 120:
            has_miniature = True
        if len(parsed_matches) > 0 and not has_miniature:
            # find the largest thumb index
            largest_match = parsed_matches[0]
            ...
            thumb_full_name = largest_match['relative_path'].split("/")[-1]
            thumb_path = os.path.join(thumb_dir, f"{thumb_full_name}")
            rel_path_small = os.path.join(".thumbs", f"{thumb_base}-160x120.png")
            thumb_path_small = os.path.join(
                thumb_dir, f"{thumb_base}-160x120.png")
            # read file
            try:
                with Image.open(thumb_path) as im:
...
def get_slicer(file_path: str) -> Tuple[BaseSlicer, Dict[str, str]]:
    ...
    log_to_stderr(f'slicer: {slicer}, ident: {ident}')
    return slicer, ident
...
def extract_metadata(
    ...
    log_to_stderr(f"extract_metadatas_licer: {slicer}")
    ...
    for key in SUPPORTED_DATA:
        log_to_stderr(f"parse_{key}")
        func = getattr(slicer, "parse_" + key)
        result = func()
        if result is not None:
            metadata[key] = result
    log_to_stderr(f"matedata: {metadata}")
    return metadata
...
def main(path: str,
    ...
    log_to_stderr(f"File path: {file_path}")
    if ufp is not None:
